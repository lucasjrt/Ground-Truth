...
namespace Ground_Truth {
    public partial class Form1 : Form {
        ...
        private void ReloadImage() {
           ...
           Console.WriteLine("Original size: " + mainImage.Width + ", " + mainImage.Height);
            Console.WriteLine("Zoom size: " + zoomSize.Width + ", " + zoomSize.Height);
            picBoxImage.SizeMode = PictureBoxSizeMode.Normal;
            picBoxImage.Size = zoomSize;
            //try {
                zoomImage = new Bitmap(mainImage, zoomSize);
                picBoxImage.Image.Dispose();
                picBoxImage.Image = CropImage(zoomImage, new Rectangle(0, 0, w, h));
                GC.Collect();
                LoadColors();
                DrawGrid();
            } catch (ArgumentException) {
                MessageBox.Show("Tamanho máximo de zoom excedido");
            try {
                cbZoom.Text = cbZoom.Items[cbZoom.SelectedIndex - 1].ToString();
